...
public class Perlin {
    ...
    /**
     * Generates Perlin noise using multiple octaves.
     *
     * @param res         The width/height of the image.
     * @param baseFreq    Base frequency for noise.
     * @param iterations  Number of iterations/octaves.
     * @return A BufferedImage containing the Perlin noise pattern.
     */
    public static BufferedImage generatePerlinNoise(int res, double baseFreq, int iterations) {
        Perlin perlin = new Perlin(new Random().nextLong());
        ...
        for (int y = 0; y < res; y++) {
            for (int x = 0; x < res; x++) {
                ...
                for (int i = 0; i < iterations; i++) {
                    noiseSum += perlin.noise(x * frequency / res, y * frequency / res) * amplitude;
                    maxValue += amplitude;
